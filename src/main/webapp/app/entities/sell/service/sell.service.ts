...
export class SellService {
  ...
  create(sell: NewSell): Observable<EntityResponseType> {
    return this.http.post<ISell>(this.resourceUrl, sell, { observe: 'response' });
  }

  update(sell: ISell): Observable<EntityResponseType> {
    return this.http.put<ISell>(`${this.resourceUrl}/${this.getSellIdentifier(sell)}`, sell, { observe: 'response' });
  }

  partialUpdate(sell: PartialUpdateSell): Observable<EntityResponseType> {
    return this.http.patch<ISell>(`${this.resourceUrl}/${this.getSellIdentifier(sell)}`, sell, { observe: 'response' });
  }
